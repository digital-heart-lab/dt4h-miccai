...
<template>
  <section id="archive" class="section-dark py-28 px-[8vw] relative overflow-hidden">
    <div class="absolute inset-0 pointer-events-none">
      <div
        class="absolute top-1/2 left-0 w-[500px] h-[500px] bg-[#1E6EF1]/[0.03] rounded-full blur-[150px] -translate-y-1/2" />
    </div>
    ...
    <div class="max-w-7xl mx-auto relative z-10">
      ...
      <div class="space-y-8">
        <template v-for="(workshop, index) in workshops" :key="workshop.year">
          <div v-if="workshop.status === 'completed'"
            class="reveal card-dark p-8 border-l-4 border-l-[#10B981] hover:border-l-[#34D399] group">
            <div class="grid lg:grid-cols-4 gap-8 items-center">
              <div class="lg:col-span-1">
                <div
                  class="font-['Space_Grotesk'] text-5xl font-bold text-[#34D399] mb-3 group-hover:scale-105 transition-transform duration-300">
                  {{ workshop.year }}</div>
          ...
          <div v-else
            class="reveal card-dark p-8 border-l-4 border-l-[#1E6EF1] hover:border-l-[#60A5FA] group relative overflow-hidden">
            <div
              class="absolute inset-0 bg-gradient-to-r from-[#1E6EF1]/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            </div>
            <div class="grid lg:grid-cols-4 gap-8 items-center relative z-10">
              <div class="lg:col-span-1">
                <div
                  class="font-['Space_Grotesk'] text-5xl font-bold text-[#60A5FA] mb-3 group-hover:scale-105 transition-transform duration-300">
                  {{ workshop.year }}</div>
                <div class="flex items-center gap-2.5 text-[#6B7280] text-sm mb-2">
                  <div class="w-7 h-7 rounded-lg bg-[#1E6EF1]/10 flex items-center justify-center">
